...
def build_transformer_model(input_shape, num_layers, d_model, dff, maximum_position_encoding, output_shape, rate=0.1):
    ...
    outputs = Dense(output_shape[-1])(enc_output1)
...
F_Net_model.load_weights('Weight_FNet_ETTh1.tf')
...
model.load_weights('Weight_EF_Net_ETTh1.tf')
test_unet = model.predict(x_test)


sec_half_model = Model(inputs=F_Net_model.get_layer('dense_9').input, outputs=F_Net_model.get_layer('dense_9').output) 
